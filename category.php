      			...
      			if ($conn)
      				{
						if (isset($_GET['id']))
						{
							$sql = "SELECT * FROM product WHERE categories_id = " . $_GET['id'] . " ORDER BY id DESC";
						...
						{
							$key = $_GET['search'];
							$sql = "SELECT * FROM product WHERE name LIKE '%$key%' ORDER BY id DESC";
							$result = mysqli_query($conn, $sql);
						...
						{
							echo '<h1>Lỗi tìm kiếm sản phẩm</h1>';
						}
					}
